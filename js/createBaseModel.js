...
function onMouseClick(event) {
  if(mouseOnMenu == false){ // If the mouse isn't in the menu
    ...
    if (mode == "buildHouse"){ // If you are building the vertices to make the building/house/wall
        ...
        lineMaterialFence = new THREE.LineBasicMaterial({color: 0x8a634a});
...
document.getElementById('buildModel').addEventListener('click', function(){
  if(confirm("Warning: Would you like to continue to the next stage? (you will not be able to edit it again).")){
    ...
    // Does the same thing as before but for the fence: (if there is a fence - if the first vertex of the fence Isn't undefined)
    
    if(pointsFence[0] != undefined){
      ...
      var geometryFence = new THREE.ExtrudeGeometry(shapeFence, extrudeSettingsFence);
      var materialFence = new THREE.MeshLambertMaterial({color: 0x8a634a});
    ...
    gridFloor.rotation.x = THREE.Math.degToRad(180);
    grassFloor.rotation.x = THREE.Math.degToRad(-90);
    camera.position.y += 5;
    camera.lookAt(0, 0, 0);
    camera.layers.disable(3);
    cameraOrbit = true;
